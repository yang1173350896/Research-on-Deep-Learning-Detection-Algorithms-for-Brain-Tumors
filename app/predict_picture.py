...
def transform(ori_img,device = 'cuda'):
    ...
    return tensor_img
def predict_picture1(input_dir,filename):
    ori_img = cv2.imread(input_dir,0) #240 240
    model_dir = "./net/label1.pth"
    output_dir ="./static/images/output1/"+filename
...
def predict_picture4(input_dir,filename):
    ori_img = cv2.imread(input_dir,0) #240 240
    model_dir = "./net/new_label4.pth"
    output_dir ="./static/images/output4/"+filename
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    net = load_net(model_dir,device)
    net = net.eval()
    ten_img = transform(ori_img,device)
